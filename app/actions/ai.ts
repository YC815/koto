...
export async function generateEntry(target: string, sentence?: string) {
  // Prompt 設計：給予明確的角色與任務
  const contextLine = sentence ? `Context Sentence: "${sentence}"` : 'Context Sentence: None';
  ...
  const prompt = `
    ...
    Instructions:
    1. Identify the reading (Furigana) of the target word. If context is provided, use the correct reading for that context (handle homonyms).
